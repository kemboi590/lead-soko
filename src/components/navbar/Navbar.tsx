...
const Navbar = () => {
    ...
    return (
        <nav className="bg-white w-[88%] mx-auto fixed top-5 left-1/2 transform -translate-x-1/2 z-50 border-2 border-gray-100 rounded-lg shadow-md">
            ...
            {/* Mobile Dropdown */}
            {isOpen && (
                <div className="lg:hidden flex flex-col bg-white shadow-md text-3xl">
                    <Dropdown title="Product" categories={navbarData.productCategories} mobile />
                    <Link to="/pricing" className="px-4 py-2 border-b">Pricing</Link>
                    <button className="px-4 py-2">Contact Sales</button>
                    <button className="border px-4 py-2 mx-4 my-2 rounded-lg">Login</button>
                    <button className="bg-blue-600 text-white px-4 py-2 mx-4 rounded-lg">Sign up free</button>
                </div>
            )}
        </nav>
    );
